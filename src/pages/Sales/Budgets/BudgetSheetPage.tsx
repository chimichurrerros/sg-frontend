...
export default function BudgetSheetPage({ mode }: BudgetSheetPageProps) {
  ...
  const { id } = useParams();
  const { data: budget, isPending: loadingBudget, isError: isErrorBudget, error: budgetError } = useCustomerQuoteById(Number(id), mode === "view");
