...
export default function ChecksList() {
    ...
    const { data: checks, isPending, isError, error } = useGetChecksKeys(params);
        ...
        <Box display="flex" flexDirection="column" gap={4} p={4} height="100%" minHeight="0">
            ...
            <Box display="flex" flexDirection="row" gap={2} justifyContent="space-between" alignItems="center">
                ...
                <Box display="flex" flexDirection="row" gap={2}>
                    ...
                    <NumberInput.Root defaultValue="10" width="70px" max={30} min={5} onValueChange={(value) => setParams({ ...params, pageSize: Number(value) })}>
                        <NumberInput.Control />
                        <NumberInput.Input />
            ...
            <Box flex="1" minHeight="0" mb={2}>
                <TableSelect
                    ...
                    // height="full"
                    minheight="0"
                    noItemsComponent={<EmptyDataScreen title="No se encontraron cheques" message="No hay cheques para mostrar en este momento." icon={<BanknoteX />} />}
